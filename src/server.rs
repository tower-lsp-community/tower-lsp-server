...
macro_rules! rpc {
    ...
    (@register $rpc_name:literal, post_cancel_request, $router:ident, $state:ident, $pending:ident) => {
        let pending_cancel_req = $pending.clone();
        $router.method_arc(
            "$/cancelRequest",
            move |state: Arc<S>, params: lsp_types::CancelParams| {
                pending_cancel_req.cancel(&params.id.clone().into());
                async move { S::post_cancel_request(&state, params).await }
            },
...
rpc! {
    ...
    pub trait LanguageServer: (Send + Sync + 'static) {
        ...
        /// The [`$/cancelRequest`] notification can be sent in both directions (in this case client to server)
        /// to cancel a previous request and avoiding unnessary computing.
        ///
        /// [`$/cancelRequest`]: https://microsoft.github.io/language-server-protocol/specification#cancelRequest
        ///
        /// When recieving a cancel request, we cancel the previous associated future and then call this handler.
